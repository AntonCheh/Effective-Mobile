...
Структура проекта
tests.effective_mobile.LoginTests.java - тестовый класс с параметризованными тестами
...
┌─────────────────────────────────────────────────────────┐
│ 1. BookingApiConfig                                      │
│    → Создает НАСТРОЙКИ (RequestSpecification)           │
│    → Говорит: "Все запросы должны идти на этот URL"     │
│    → Говорит: "Все запросы должны иметь Content-Type"   │
└─────────────────────────────────────────────────────────┘
↓ передает настройки
┌─────────────────────────────────────────────────────────┐
│ 2. BookingApiClient                                      │
│    → Получает настройки в конструкторе                  │
│    → Использует эти настройки ДЛЯ ЗАПРОСОВ              │
└─────────────────────────────────────────────────────────┘
↓ использует
┌─────────────────────────────────────────────────────────┐
│ 3. RestAssured (given/when/then)                        │
│    → Реально ОТПРАВЛЯЕТ HTTP запрос                     │
